...
function cargaPortfolio(){
	...
	$ps = get_posts(array('post_type' => 'post' , 'numberposts' => -1 , 'tipo' => $tipo ,'disciplina' => $disciplina , 'artistas' => $artista));
    
	foreach($ps as $p):
	
		$disciplinas = wp_get_post_terms( $p->ID, 'disciplina' );
		$artistas = wp_get_post_terms( $p->ID, 'artistas' );
		$tipos = wp_get_post_terms( $p->ID, 'tipo' );?>

		<article id="post-<?php echo $p->ID; ?>" <?php //post_class('postblog'); ?> class="postblog post <?php echo 'at-'.$artistas[0]->term_id;?> <?php echo 'pd-'.$disciplinas[0]->term_id;?> <?php echo 'td-'.$tipos[0]->term_id;?>">
			<header class="entry-header">
				<h2 class="entry-title">
                	<a href="<?php echo get_permalink($p->ID); ?>" title="Ver <?php echo $p->post_title?>" rel="bookmark">
						<?php echo $p->post_title ?>
                    </a>
                </h2>
                <h3 class="entry-title">
					<?php echo $artistas[0]->name ?>
                </h3>	
            	<div class="entry-postformat">
                	<a href="<?php the_permalink(); ?>">ver post |</a>
			 		<?php echo $tipos[0]->name ?>   |  
			 		<?php echo $disciplinas[0]->name ?> | 
				</div>
				<div class="entry-postformat2"><?php echo get_the_date(); ?></a><?php _e('', 'renkon') ?></div>
			</header>

			<?php 	echo '<a href="'; get_permalink($p->ID); echo '" class="thumb">';
...
add_action('wp_ajax_cargaPlus', 'cargaPlus');
add_action('wp_ajax_nopriv_cargaPlus', 'cargaPlus');
...
if(function_exists("register_field_group"))
{
	register_field_group(array (
		...
		'fields' => array (
			array (
				'key' => 'field_55c98c1df43f6',
				'label' => 'Disciplina',
				'name' => 'disciplina',
				'type' => 'taxonomy',
				'instructions' => 'Seleccione la Disciplina que ejerce el artista',
				'taxonomy' => 'disciplina',
				'field_type' => 'radio',
				'allow_null' => 0,
				'load_save_terms' => 0,
